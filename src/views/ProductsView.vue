            ...
            <div class="col" v-for="product in products" :key="product.id">
              ...
              <div class="card border-0 mb-4 position-relative position-relative">
                ...
                <div class="card-body p-0">
                  <h4 class="mb-0 mt-3 text-center">
                    <router-link :to="`/product/${product.id}`">{{ product.title }}</router-link>
                  </h4>
                  <p class="card-text mb-0 text-center">NT{{ product.origin_price }} <span class="text-muted text-center"><del>NT{{ product.price }}</del></span></p>
                  <p class="text-muted mt-3 text-center" style="height: 4.5rem; overflow: hidden;">{{ product.description }}</p>
                </div>
              </div>
            </div>
          </div>
